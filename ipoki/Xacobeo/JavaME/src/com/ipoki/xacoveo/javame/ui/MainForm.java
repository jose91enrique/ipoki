...
import com.ipoki.xacoveo.javame.web.WebResult;
import com.ipoki.xacoveo.javame.web.XacoveoWeb;
...
public class MainForm extends Form implements CommandListener {
	...
	public void commandAction(Command command, Displayable displayable)
    {
		...
		else if ( command == cmdDisconnect)
		{
			if (isConnected)
			{
				...
				{
					isConnected = false;
					XacoveoWeb.sendWebDisconnection();
					strComStatus.setText("Disconnected");
					img.setImage(imageOff);
					webTimerTask.cancel();
	...
	private boolean connectToWeb()
	{
		...
		try
		{
			int webResult =  XacoveoWeb.sendWebReg(State.user, State.password);
			Alert alertScreen;
			switch(webResult)
			{
			...
			case WebResult.ALERT_OK:
				if (XacoveoAlert.IsPositional)
				{
					Date hoy = new Date();
					String textoAviso = Tools.fechaToString(hoy.getTime()) + "\n" +
											XacoveoAlert.Login + " is at " + XacoveoAlert.Distance + " meters.";
				...
				else
				{
					...
					String textoAviso = Tools.fechaToString(hoy.getTime()) + "\n" +
											XacoveoAlert.Text;
					strFromUserData.setText(XacoveoAlert.Login);
					strDistanceData.setText(XacoveoAlert.Distance +  " meters");
					strAlert.setText(textoAviso);
				}
				result = true;
				break;
    	...
    	if (State.positionSource.equals("GPS"))
    	{
    		if (gpsConnected)
    		{
    			...
    			if (Math.abs(latitude - lastSentLat) > 0.00001 || Math.abs(longitude - lastSentLon) > 0.00001)
    			{
    				try
    				{
    					XacoveoWeb.sendWebPos(strLatitudeData.getText(), strLongitudeData.getText());
    				}
    				catch(IOException ioe)
    				{
    					isConnected = false;
    ...
    private Image getMap(int height, int width) throws IOException
    {
    	...
    	return XacoveoWeb.sendWebRequestImage(url);
    }
}
